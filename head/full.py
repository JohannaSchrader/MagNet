...
import models.chainer_file, models.binary_classifier_adapter, models.sample_weight_adapter, models.one_hot_classifier_adapter, models.one_hot_target_adapter, models.xgboost_file
#from models import chainer_file,binary_classifier_adapter,sample_weight_adapter,one_hot_classifier_adapter,one_hot_target_adapter,xgboost_file
import tracemalloc
import os
import numpy as np
...
import head.logger as logger
import features.knn
import argparse

parser = argparse.ArgumentParser()
parser.add_argument('--input_dir', default='../micro_rna/20160809_contestant/')
parser.add_argument('--out_dir', default='/home/schrader/our_head/head/timecomplexity/fold0')
parser.add_argument('--percent', default=1.0)
    ...
    for seed in seeds:
        ...
        pl.predict_proba(
            ...
            estimator=xgboost.XGBClassifier(
                objective='multi:softproba', learning_rate=0.05, max_depth=5, n_estimators=1000,
                nthread=10, subsample=0.5, colsample_bytree=1.0),
        )
        pl.predict_proba(
            output_name='lev1_mlp3', input_names=['imputed', 'tsne'], validate_size=2,
            estimator=models.chainer_file.ChainerClassifier(models.chainer_file.MLP3, gpu=args.gpu, n_epoch=100, n_out=len(pl.label_names))
        )
        pl.predict_proba(
            output_name='lev1_mlp4', input_names=['imputed', 'tsne'], validate_size=2,
            estimator=models.chainer_file.ChainerClassifier(models.chainer_file.MLP4, gpu=args.gpu, n_epoch=200, n_out=len(pl.label_names))
        )
...
if __name__ == '__main__':
    ...
    print('done main')
    #balancedaccuracy = metrics_.balanced_accuracy_score(y_true, y_pred)
    fname = os.path.join(args.out_dir,args.percent,'runtime.txt')
    with open(fname, 'w') as f:
        f.write('time and memory:')
        f.write('runtime: '+str(runtime))
        f.write('full memory usage: '+str(tracemalloc.get_traced_memory()))
        f.write('check')
